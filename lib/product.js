...
export async function updateMinStock(productId, data) {
    const response = await authenticatedFetch(`${API_BASE_URL}/products/${productId}/min-stock`, {
        method: "POST",
        headers: {
    ...
    if (!response.ok) {
        throw new Error("Failed to update minStock");
    }
    return response.json();
}